...
describe("Dappazon", () => {
  ...
  describe("Listing", () => {
    ...
    it("Returns item attributes", async () => {
      ...
      expect(item._name).to.equal(NAME)
      expect(item._category).to.equal(CATEGORY)
      expect(item._image).to.equal(IMAGE)
      expect(item._cost).to.equal(COST)
      expect(item._rating).to.equal(RATING)
      expect(item._stock).to.equal(STOCK)
    })
